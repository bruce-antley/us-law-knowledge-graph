...
VALID_EDGE_TYPES = {
    ...
    'PRECONDITION_TO',  # v0.5: DoctrinalTest/Doctrine -> DoctrinalTest threshold gate
}
...
VALID_OVERRULE_TYPES = {'explicit', 'implicit', 'effective'}
...
VALID_CASE_STATUSES = {
    ...
    # v0.5 additions:
    'per_curiam_affirmed',   # affirmed by equally divided Court, no precedential effect
    'plurality_superseded',  # controlling plurality opinion superseded in practice by later majority
}

VALID_PRECEDENTIAL_WEIGHTS = {'binding', 'persuasive', 'none'}

VALID_SCRUTINY_LEVELS = {
    'strict', 'intermediate', 'rational_basis', 'balancing',
    'per_se', 'heightened', 'none',
    'exacting_scrutiny',  # v0.5: NAACP v. Alabama line, compelled disclosure cases
    'categorical',          # categorical rule, e.g. Miller obscenity test (not true scrutiny)
}
...
EDGE_COMPATIBILITY = {
    ...
    'PRECONDITION_TO':         {  # v0.5: threshold gate between tests
        'DoctrinalTest': {'DoctrinalTest'},
        'Doctrine': {'DoctrinalTest'},
    },
}
...
def validate_doctrine(node, r):
    ...
    # v0.5: secondary_area_ids validation
    secondary = node.get('secondary_area_ids', [])
    if secondary:
        if not isinstance(secondary, list):
            r.error('E001', "secondary_area_ids must be an array", loc)
        elif node.get('area_id') in secondary:
            r.warning('W005', f"secondary_area_ids contains primary area_id '{node['area_id']}'", loc)
...
def validate_doctrinal_test(node, r):
    ...
    if node.get('scrutiny_level') and node['scrutiny_level'] not in VALID_SCRUTINY_LEVELS:
        r.error('E006', f"scrutiny_level '{node['scrutiny_level']}' not in valid enum", loc)
